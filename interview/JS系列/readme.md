...
- 是什么：
  1. 内部函数或对象对外层函数的变量（包括window）存在引用，这些变量的集合叫做闭包。当一个函数被执行，这个函数会被推入调用栈，当这个函数抛出一个内部函数，这个内部函数存在对外部函数变量引用导致外部函数的变量会被打包入堆内存。
  2. 闭包情况下使用eval和with会将全部变量打包入堆。
- 使用场景：
...
1. 原型链继承
   - 通过new的原理，将构造函数的原型绑定到另一个构造函数的prototype属性
   - 无法给父类灵活
   - 多个实例对象共用同一个原型会相互影响
...
# 22.CommonJS 和 ESModule

### CommonJS

1. 模块是对象，原始类型是值的拷贝，引用类型是引用地址,可以修改引用类型的属性或给引用类型重新赋值，重新赋值后会是新的变量

```
let obj = require('./b.js') // 这只是赋值给当前的新变量，相当于将b中的obj的引用地址拿到
obj = {} //新的变量
```

2. 加载模块是同步的，执行时加载，多次导入执行一次
3. 循环依赖安全，不支持顶层 await（CommonJS同步解析模块是会包装成一个函数，函数不存在async，会报错）
   - 循环依赖：文件中依赖相互引用并执行，为什么循环依赖安全？
     - 存在缓存机制，先读取缓存;读取文件立即创建一个空的导出对象

   ```
   const b = require('./b')
   console.log('a拿到b:', b.num)
   exports.num = 100


   const a = require('./a')
   console.log('b拿到a:', a.num)
   exports.num = 200



   import fs from 'fs/promises'
    // 顶层 await，是在ESModule的特性
...
- 执行流程：
  ...
  4. 包装成函数执行
     ...
     - 创建module
     - 创建空的导出对象：module.exports = {}，放入缓存

  5. 执行包装函数
     - 同步执行模块代码，给module.exports对象赋值
  6. 返回导出对象
     - 返回最终的module.exports

- commonjs是同步执行，为什么在require后读取会存在属性undefined。
  当读取执行a文件是，中途遇到b文件的导入，会暂停a文件的执行，此时b文件执行途中又遇到a文件的require，会先去读取缓存，此时a文件的变量还没被赋值，缓存拿到的就会说undefined。
